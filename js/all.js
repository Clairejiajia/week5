...
$(document).ready(function () {
  ...
  var len = pathNameArray.length;
  var target = pathNameArray[len - 1].substring(0, 5);
...
var assignmentData = [{
  ...
  file: 'HW-Emily.zip',
  discuss: 'Hi Teacher, the attached file is my homework of chapter 2. And here comes a question: Why in the following code the height of the <code>div</code> is bigger than the height of the <code>img</code> ? There is a gap below the image, but it doesn’t seems to be a padding/margin.<br> What is the gap or extra space below image?',
  reply: {
    replied: true,
    ...
    discuss: 'Hi David, by default, an image is rendered inline, like a letter soitsits on the same line that a, b, c and d sit on.There is space below that line for the descenders you find on letterslike g, j, p and q.<br><br><br>You can:<br><br><ul><li>adjust the<code>vertical-align</code> of the image to position it elsewhere (e.g. the <code>middle</code> )or</li><li>change the <span>display</span> so it isn’t inline.</li></ul>'
  }
}, {
